...
check_ssh_auth_sock() {
    ...
    grep "SSH_AUTH_SOCK" /proc/*/environ 2>/dev/null
    grep "SSH_AUTH_SOCK" /proc/*/*/environ 2>/dev/null
}

# Function to check running processes for KRB5CCNAME
check_krb5ccname() {
    echo "===== [Checking all running processes for KRB5CCNAME in environ through /proc] ==================================================================="
 grep "KRB5CCNAME" /proc/*/environ 2>/dev/null | awk '{print $3}' | xargs -I {} sh -c 'echo "-----------------\n{}\n-----------------"; cat {};echo "\n-----------------"'
}
