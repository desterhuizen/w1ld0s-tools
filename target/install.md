...
It is `target/target.py` (Python 3.11 or newer — it uses `tomllib` from the stdlib). No pip dependencies.
...
`setup_links` symlinks every executable in the repo into `~/.local/bin/`, stripping the file extension — so `target/target.py` becomes `~/.local/bin/target`. Run it once to initialise the configuration:
...
```bash
chmod +x target/target.py
ln -s "$PWD/target/target.py" ~/.local/bin/target
```
...
Configuration lives in `target/config/`, next to the script itself. Two kinds of file:
...
2. `target/config/` next to the real script (symlinks resolved).
...
Edit `target/config/config.toml`:
...
To customize the notes themselves, edit `target/templates/host-notes.md`. Keep the `## N.` headings intact — the `-F`/`-A`/`-H`/`-X` append commands locate sections by those headings and will refuse to write if one is missing.
...
**Config adoption.** The old bash implementation resolved its own directory with `cd "$(dirname "${BASH_SOURCE[0]}")" && pwd`, which does not follow symlinks. Invoked through `~/.local/bin/target`, it therefore stored everything in `~/.local/bin/config/` rather than `target/config/`. On first run, `target.py` finds that directory and **copies** (never moves) the files to the canonical location, then tells you the `rm -rf` command to clean up. Copying means a rollback still finds its config where it expects.
...
**Rollback** is documented in `target/legacy/README.md`. The state files keep their shell format specifically so the frozen bash remains a drop-in.

## Troubleshooting

* **Permission denied** — `chmod +x target/target.py`.
...
* **`$IP` is empty in your shell** — open a new shell, or `source ~/.zshrc`. The variables are exported by `aliases` at startup, from `target/config/*.sh`.
